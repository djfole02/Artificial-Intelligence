...
Project 5 10/30/2021

Traveling Salesperson Problem - find the shortest route using GA+WoC
*/
...
struct graph {
	...
	bool check_if_in(int num) { //see if a city number is not contained in the cities_out vector, if it is = return false;
		for (int i = 0; i < cities_in.size(); i++) {
			if (num == cities_in[i].num) {
				return true;
			}
		}
		return false;
...
struct dna_results {
	...
	vector<graph> swap_mutation() {//goes through population and swaps two cities
		...
		for (int i = 0; i < m.size(); i++) {
			int rate = rand() % 3; //there is a 33% mutation rate
			if (rate == 1) {
				//cout << "MUTATING" << endl;
	...
	vector<graph> inverse_mutation() {//takes a section of link 5 in the tour and reveres
		...
		for (int i = 0; i < m.size(); i++) {
			int rate = rand() % 3; //33% mutation rate
			if (rate == 1) {
				//cout << "MUTATING" << endl;
...
void makeGraph(struct graph g, int gen, string savefile) {
	...
	if (gen != -1) {
		...
		string dist;
		if (gen == 0) {
			dist = "Wisdom of Crowds\nDistance: " + to_string(g.dist);
		}
		else {
			dist = "Generation: " + to_string(gen) + "\n" + "Distance: " + to_string(g.dist);
		}
...
dna_results crossover_method(dna_results dna, string crossover) { //returns two children populations from two parent populations.
	...
	while (1) {
		int c1 = rand() % (int)(dna.pop.size()*.1); //takes from the top 20%
		int c2 = rand() % (int)(dna.pop.size()*.5); //takes from the top 70%
...
int main() {
	...
	string mut = "swap_mutation";  //mutation method (can either be "inverse_mutation" or "swap_mutation")
	int iterations = 100;			  //number of iterations to do until no progress is made.
	int population_size = 100;
	graph woc_g;
	woc_g.insert_cities(Info);
	vector<dna_results> best_results = dna(Info, mut, cross, iterations, population_size);
	auto wocStart = chrono::steady_clock::now();
	woc(best_results, &woc_g);
	...
	for (int i = 0; i < best_results.size(); i++) {
		node value;
		value.num = i;
		value.x = best_results[i].generation;
		value.y = best_results[i].pop[0].dist;
		g.all_cities.push_back(value);
	}
	printf("GA ONLY Elapsed Time in ms: %.2f\n", (double)chrono::duration_cast<chrono::milliseconds>(wocStart - dnaStart).count());
	cout << "Best GA Generation: " << best_results.back().generation << endl;
	cout << "Best GA Distance: " << best_results.back().pop[0].dist << endl;
	cout << "Best GA Route: " << endl;
	best_results.back().pop[0].print_tour();
	
	printf("\n\nGA + WOC Time in ms: %.2f\n", (double)chrono::duration_cast<chrono::milliseconds>(dnaEnd - dnaStart).count());
	cout << "WOC Distance: " << woc_g.dist << endl;
	cout << "WOC Route: " << endl;
	woc_g.print_tour();
	system("taskkill /F /IM plot.exe >nul 2>&1");
	...
	makeGraph(woc_g, 0, "woc");
	cout << "\nRun the Python Code to Create a Gif, Once Created Then: " << endl;
	system("pause");
